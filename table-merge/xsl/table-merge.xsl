...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
          ...
          <xsl:choose>
            <xsl:when test="current-group()[1][self::*:table[not(hub:is-continued-table(.))]]">
              <xsl:for-each-group select="current-group()" group-ending-with="*[hub:is-continued-table(.)]">
                <xsl:choose>
                  <xsl:when test="current-group()[1][self::*:table[not(hub:is-continued-table(.))]] and current-group()[last()][self::*[hub:is-continued-table(.)]]">
                                              ...
                                              <xsl:choose>
                                                <xsl:when test="count(current-group()) gt 1">
                                                  ...
                                                  select="$colspecs[@colname=current()][1]/@colnum"/>
                                                        <xsl:attribute name="colwidth" select="hub:get-average-width($colspecs[@colname=current()]/@colwidth)"/>
                                                      </colspec>
                                                    </xsl:for-each>
                                                    <xsl:for-each
                                                  ...
                                                  mode="#current"/>
                                                    ...
                                                    <tbody>
                                                      <xsl:apply-templates select="current-group()/*:tbody/*:row[not(not(preceding-sibling::*:row) and  hub:continuation-marked-in-first-row(.))]" mode="#current"/>
                                                    </tbody>
                                                  </tgroup>
                                                </xsl:when>
                                                <xsl:otherwise>
  ...
  <xsl:function name="hub:is-continued-table" as="xs:boolean">
    ...
    <xsl:choose>
      <xsl:when test="$item[self::*:informaltable[*:tgroup[1]/(*:thead, *:tbody)[1]/*:row[1][hub:continuation-marked-in-first-row(.)]]]">
  ...
  <xsl:function name="hub:get-average-width" as="xs:string">
    ...
    <xsl:value-of select="concat((sum(for $i in $values return tr:length-to-unitless-twip($i)) div count($values)) div 56.6929, 'mm')"/>
  </xsl:function>
