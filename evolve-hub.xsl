...
<xsl:stylesheet
  ...
  <xsl:import href="http://transpect.io/evolve-hub/xsl/catch-all/catch-all.xsl"/>

  <xsl:import href="http://transpect.io/evolve-hub/xsl/figure-captions/figure-captions.xsl"/>
  <xsl:import href="http://transpect.io/evolve-hub/xsl/hierarchy-by-role/hierarchy-by-role.xsl"/>
  <xsl:import href="http://transpect.io/evolve-hub/xsl/lists-by-indent/lists-main.xsl"/>
  <xsl:import href="http://transpect.io/evolve-hub/xsl/table-captions/table-captions.xsl"/>
  <xsl:import href="http://transpect.io/evolve-hub/xsl/relocate-indexterms/relocate-indexterms.xsl"/>
  <xsl:import href="http://transpect.io/evolve-hub/xsl/table-merge/table-merge.xsl"/>
 ...
 <!-- if sidebars above are dissolved also paras whose only purpose is to reference such a sidebar can be discarded -->
  ...
  <xsl:template match="*:sidebar[matches(@role, $hub:split-style-regex)][not(preceding-sibling::*[self::sidebar[matches(@role, $hub:split-style-regex)]])]" mode="hub:join-tables">
    <xsl:copy>
      <xsl:apply-templates select="@*" mode="#current"/>
      <xsl:apply-templates select="node() except node()[matches(@role, $hub:split-style-regex)]" mode="#current"/>
      <xsl:if test="*[last()][matches(@role, $hub:split-style-regex)]">
       <xsl:call-template name="join-splitted-paras-in-sidebars"/>
      </xsl:if>
      <xsl:choose>
        <xsl:when test="following-sibling::*[self::sidebar[matches(@role, $hub:split-style-regex)]]">
          <xsl:apply-templates select="following-sibling::*[1]" mode="#current">
            <xsl:with-param name="merged-sidebar" select="'yes'" as="xs:string" tunnel="yes"/>
          </xsl:apply-templates>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="*:sidebar[preceding-sibling::*[self::sidebar[matches(@role, $hub:split-style-regex)]]]" mode="hub:join-tables">
    <xsl:param name="merged-sidebar" as="xs:string?" tunnel="yes"/>
  ...
  <xsl:template match="para[parent::*[preceding-sibling::*[1]
                                                          [matches(@role, $hub:split-style-regex)]/*[matches(@role, $hub:split-style-regex)]
                                                                                                    [position() = last()]
                                     ]
                             ]" mode="hub:join-tables">
    <xsl:param name="merged-sidebar" as="xs:string?" tunnel="yes"/>
    <xsl:if test="$merged-sidebar != 'yes'">
      <xsl:next-match/>
    </xsl:if>
 ...
 <!-- Wouldn't it be useful to discard the linkend then (I did that now – Maren)? It is already dissolved and needs an id which is discarded in next template -->
  ...
  <xsl:variable name="hub:wingdings-font-map" select="if (doc-available('../fontmaps/Wingdings.xml')) then document('../fontmaps/Wingdings.xml')/symbols/symbol else ()" as="node()*"/>
  ...
  <xsl:variable name="hub:symbol-font-map" select="if (doc-available('../fontmaps/Symbol.xml')) then document('../fontmaps/Symbol.xml')/symbols/symbol else ()" as="node()*"/>
  ...
  <xsl:template match="title[not(parent::formalpara | parent::sidebar) and not(@role)][../@role]" mode="hub:repair-hierarchy">
    <xsl:copy>
      <xsl:apply-templates select="@*" mode="#current" />
  ...
  <xsl:template match="*[not(self::formalpara | self::sidebar)][@role][title[not(@role)]]/@role" mode="hub:repair-hierarchy" />
...
<!--   </xsl:template> -->
                             ...
                             [
                               ...
                               and (
                                ...
                                (: preceding-sibling::node()[not(self::text()[matches(., '^\s+$')])][1]/self::anchor[. is ancestor::*[self::para or self::tocentry][1]//node()[not(self::text()[matches(., '^\s+$')])][1]] :) 
                                 every $item in preceding-sibling::node() satisfies ($item/self::text()[matches(., '^\s+$')] or $item/self::anchor) 
  ...
  <xsl:template match="text()
                         ...
                         and (
                           ancestor::para[xs:double(@margin-left) gt $hub:indent-epsilon][
                             ...
                             or
                             (.//tab)[1][preceding-sibling::node()[1][. is current()]]
                           ]
                           or
                           ancestor::tocentry
    ...
    <xsl:copy>
      ...
      <xsl:choose>
        ...
        <xsl:when test="( node()/self::phrase[matches(., $hub:caption-number-without-sep-regex)]
                                             [not(phrase[@role = 'hub:identifier'])]
                                             [every $n in preceding-sibling::node() satisfies $n/self::anchor]
  ...
  <xsl:template match="entry | footnote | listitem | figure" mode="hub:upward-project-br">
    <xsl:param name="restricted-to" as="node()+" tunnel="yes" />
    <xsl:if test="exists(. intersect $restricted-to)">
