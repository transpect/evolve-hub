...
<xsl:variable name="hub:startendbox-formalpara-heading-role-regex" as="xs:string"
  ...
  <xsl:variable name="hub:pstyle-regex" as="xs:string"
   ...
   <xsl:variable name="hub:create-several-table-groups" as="xs:boolean" select="true()" />
  ...
  <xsl:template match="tgroup[tbody/row/entry/para[matches(@role, $hub:thead-pstyle-regex)]]
    ...
    <xsl:variable name="heads" select="tbody/row[hub:is-first-thead-row(.)]"/>
    
    <xsl:choose>
      <xsl:when test="count($heads) gt 1 and $hub:create-several-table-groups">
        <!-- if several heads exist in table they can be moved into several tgroup elements. 
          if this is not wanted, set $hub:create-several-table-groups to false() in your project, 
          then only the first head rows will be mapped to thead-->
        <xsl:for-each-group select="tbody/row" group-starting-with="row[hub:is-first-thead-row(.)]">
          <tgroup>
            ...
            <xsl:if test="current-group()[$heads[last()]]">
              <!-- not sure where to put tfoot in this case correctly or whether this works due to missing test data-->
              <xsl:apply-templates select="tfoot" mode="#current"/></xsl:if>
          </tgroup>
        </xsl:for-each-group>
      </xsl:when>
      <xsl:otherwise>
...
<!--  <xsl:variable name="hub:caption-sep-among-caption-number-and-caption-text-regex" as="xs:string"
  ...
  <xsl:function name="tr:exception-from-identifier-detection" as="xs:boolean">
    <xsl:param name="context" as="node()"/>
    <xsl:sequence select="if (matches($context/parent::*/@role, $hub:no-identifier-needed)) then true() else false()"/>
  </xsl:function>
 ...
 <xsl:template match="tabs/tab/@alignment-char[. = '']" mode="hub:clean-hub"/>
  ...
  <xsl:variable name="hub:url-regex" as="xs:string"
    select="'(www\.[a-zA-ZäÄöÖüÜßẞ\p{IsArabic}\p{IsHebrew}\p{IsCyrillic}\p{IsGreek}][-a-zA-ZäÄöÖüÜßẞ\p{IsArabic}\p{IsHebrew}\p{IsCyrillic}\p{IsGreek}0-9.]+\.[-a-zA-ZäÄöÖüÜßẞ\p{IsArabic}\p{IsHebrew}\p{IsCyrillic}\p{IsGreek}0-9+&amp;@#/%?=~_|!:,.;\[\]]*[-a-zA-ZäÄöÖüÜßẞ\p{IsArabic}\p{IsHebrew}\p{IsCyrillic}\p{IsGreek}0-9+&amp;@#/%=~_|\[\]])|(doi\s?:\s*)?((https?|ftp|file|rtsp)://[-a-zA-ZäÄöÖüÜßẞ\p{IsArabic}\p{IsHebrew}\p{IsCyrillic}\p{IsGreek}0-9+&amp;@#/%?=~_|!:,.;\(\)\[\]]*[-a-zA-ZäÄöÖüÜßẞ\p{IsArabic}\p{IsHebrew}\p{IsCyrillic}\p{IsGreek}0-9+&amp;@#/%=~_|\[\]])'"/>
  ...
  <xsl:template match="para[phrase[@xml:lang][not(matches(@role,$semantic-phrase-roles))]]
                      |term[phrase[@xml:lang][not(matches(@role,$semantic-phrase-roles))]]
                      |bibliomixed[phrase[@xml:lang][not(matches(@role,$semantic-phrase-roles))]]" mode="hub:repair-language-markup">
    <xsl:copy>
      <xsl:apply-templates select="@*" mode="#current"/>
      <xsl:for-each-group select="node()" 
